...
const manifestFile = (filePath, commandLine, fileartifactID, targetRepo, version, label1, label2, label3, label4) => {
  ...
  //to create the index file for manifest lable thing ----------
  console.log(`${targetRepo}\\Manifest\\ManifestIndex.txt`)
   if (!fs.existsSync(`${targetRepo}\\Manifest\\ManifestIndex.txt`)){
         ...
         fs.appendFile(`${targetRepo}\\Manifest\\ManifestIndex.txt`, bufferindex, function (err) {
           if (err) {
             fs.readFile(`${targetRepo}\\Manifest\\ManifestIndex.txt`, function (err, data) {
               if (err) throw err;
               if((data.indexOf(label1)) == -1 || (data.indexOf(label2)) == -1 ||(data.indexOf(label3)) == -1 ||(data.indexOf(label4)) == -1){
                 return console.log(err);
               }
              });
           }
