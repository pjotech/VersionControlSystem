...
router.post('/checkout', function (req, res) {
  ...
  if (req.body.path === "") {
    // list of manifest version label
    let ph = "";
    ...
    fs.readdir(req.body.srcpath, function (err, files) {
      ...
      fs.readFile(manifestIndexpath, function (err, data) {
        ...
        fs.readFile(targetmanifestName, function (err, data) {
          if (err) { console.log(err) }//read manifest file
...
function createMRandMT(source_path, tar_get, file, ManifestArtifactArray, fileArray, targetroot, sourcelabel, searchlabel, repo) {
  ...
  artifactID(`${source_path}` + "\\" + file).then((artifactID) => {
    if (ManifestArtifactArray.indexOf(artifactID) >= 0 && fileArray.indexOf(file) >= 0) {
      //skip as source and target is same
    }
      ...
      let f = path.basename(source_fileName, path.extname(source_fileName)) + "_MR";
      ...
      commonAncestor(sourcelabel, searchlabel, repo).then((commonAncestor) => {
        ...
        return findnextManifest(grandma, repo).then((file1) => {
          ...
          fs.readFile(grand_manifestpath, function (err, data) {
            if (err) { console.log(err) }//read manifest file
            let dataArray2 = data.toString().split(/[ =\n\s+]+/);
            ...
            let fGrandmaMG = path.basename(file, path.extname(file)) + "_MG";
            let MG_file = fGrandmaMG + path.extname(file);
            fs.access(`${tar_get}`, (err) => {
              if (err) {  // console.log(err)
                fs.mkdirSync(`${target}`);
              }
              copyFilewe(`${tar_get}\\` + target_fileName, path.join(`${tar_get}`, `${MT_file}`));//copy MT target
              copyFilewe(`${source_path}\\` + source_fileName, path.join(`${tar_get}`, `${MR_file}`));//copy MR target
              copyFilewe(`${ggLOop}`, path.join(`${tar_get}`, `${MG_file}`));//copy MG file
              ///-----------Create manifest for merge out-----------------------------------------------
      ...
      ///---------------end of grandma calling part -------------------------------------------
    }
  }, (err) => {
    console.log(err)
  });
}
//  -----------------Find manifest File for Grandma finding--------------------------------------------------
...
//------------------------Find Ancestor File for Grandma finding--------------------------------------------------
